          ...
          {operators.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-muted-foreground text-center mb-4">
                No operators have been registered in the Factory contract yet.
              </p>
              <Button asChild>
              ...
              <TableBody>
                {operators.map((operator, index) => (
                  <TableRow key={`${operator.operator}-${index}`}>
                    <TableCell>
                      <Link
                        href={`/dashboard/operator/${operator.operator}`}
                        className="font-medium hover:underline cursor-pointer font-mono "
                      >
