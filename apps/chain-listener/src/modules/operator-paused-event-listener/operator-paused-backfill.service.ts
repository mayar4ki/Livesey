...
@Injectable()
export class OperatorPausedBackfillService implements OnModuleDestroy {
  private readonly logger = new Logger(OperatorPausedBackfillService.name);
  ...
  constructor(
    ...
    private readonly queueService: OperatorPausedQueueService,
  ) {}
  ...
  async reconcile() {
    ...
    const operatorPausedEvent = FactoryAbi.find((item) => item.type === 'event' && item.name === 'OperatorPaused');

    if (!operatorPausedEvent) {
      this.logger.error('OperatorPaused event ABI not found; skipping backfill');
      return;
    }
    ...
    while (cursor <= toBlock) {
      ...
      this.logger.debug(
        `OperatorPaused backfill chunk ${cursor}→${chunkEnd} fetched=${logs.length} filtered=${filteredLogs.length}`,
      );
  ...
  initiateContinuousReconciliation(intervalMs?: number) {
    ...
    this.reconcileTimer = setInterval(() => {
      ...
      this.reconcile()
        .catch((error) => {
          this.logger.error(
            'OperatorPaused reconciler error',
