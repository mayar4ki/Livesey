...
@Injectable()
export class TokenCreatedBackfillService implements OnModuleDestroy {
  private readonly logger = new Logger(TokenCreatedBackfillService.name);
  ...
  constructor(
    ...
    private readonly queueService: TokenCreatedQueueService,
  ) {}
  ...
  private getConfig() {
    ...
    return {
      ...
      defaultBlockRange: defaultBlockRangeEnv ? BigInt(defaultBlockRangeEnv) : 50n,
    };
  }
  ...
  async reconcile() {
    ...
    const tokenCreatedEvent = FactoryAbi.find((item) => item.type === 'event' && item.name === 'TokenCreated');

    if (!tokenCreatedEvent) {
      this.logger.error('TokenCreated event ABI not found; skipping backfill');
      return;
    }
    ...
    while (cursor <= toBlock) {
      ...
      this.logger.debug(
        `TokenCreated backfill chunk ${cursor}→${chunkEnd} fetched=${logs.length} filtered=${filteredLogs.length}`,
      );
