...
@Injectable()
export class OrderFilledBackfillService implements OnModuleDestroy {
  private readonly logger = new Logger(OrderFilledBackfillService.name);
  ...
  private getConfig() {
    const chunkSizeEnv = this.configService.get<number>('ORDER_FILLED_BACKFILL_CHUNK_SIZE', { infer: true });
    const intervalEnv = this.configService.get<number>('ORDER_FILLED_BACKFILL_INTERVAL_MS', { infer: true });
    const defaultBlockRangeEnv = this.configService.get<number>('ORDER_FILLED_BACKFILL_DEFAULT_BLOCK_RANGE', {
      infer: true,
    });
  ...
  async reconcile() {
    ...
    const orderFilledEvent = ONEINCH_LIMIT_ORDER_PROTOCOL_ABI.find(
      (item) => item.type === 'event' && item.name === 'OrderFilled',
    );

    if (!orderFilledEvent) {
      this.logger.error('OrderFilled event ABI not found; skipping backfill');
      return;
    }
    ...
    while (cursor <= toBlock) {
      ...
      this.logger.debug(
        `OrderFilled backfill chunk ${cursor}→${chunkEnd} fetched=${logs.length} filtered=${filteredLogs.length}`,
      );
  ...
  initiateContinuousReconciliation(intervalMs?: number) {
    ...
    this.reconcileTimer = setInterval(() => {
      ...
      this.reconcile()
        .catch((error) => {
          this.logger.error('OrderFilled reconciler error', error instanceof Error ? error.stack : String(error));
        })
        .finally(() => {
