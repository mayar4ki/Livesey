...
@Injectable()
export class OrderCancelledBackfillService implements OnModuleDestroy {
  private readonly logger = new Logger(OrderCancelledBackfillService.name);
  ...
  async reconcile() {
    ...
    const orderCancelledEvent = ONEINCH_LIMIT_ORDER_PROTOCOL_ABI.find(
      (item) => item.type === 'event' && item.name === 'OrderCancelled',
    );

    if (!orderCancelledEvent) {
      this.logger.error('OrderCancelled event ABI not found; skipping backfill');
      return;
    }
    ...
    while (cursor <= toBlock) {
      ...
      this.logger.debug(
        `OrderCancelled backfill chunk ${cursor}→${chunkEnd} fetched=${logs.length} filtered=${filteredLogs.length}`,
      );
  ...
  initiateContinuousReconciliation(intervalMs?: number) {
    ...
    this.reconcileTimer = setInterval(() => {
      ...
      this.reconcile()
        .catch((error) => {
          this.logger.error(
            'OrderCancelled reconciler error',
