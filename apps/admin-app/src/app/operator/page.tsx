...
export default function Page() {
  ...
  const { data, isLoading } = useOperatorList({ chainId, skip: params.skip, take: params.take, search: params.search });
            ...
            {operators.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-muted-foreground text-center mb-4">
                  {params.search ? 'No operators match your search criteria.' : 'No operators have been registered yet.'}
                </p>
                {!params.search && (
                  <Button asChild>
                    <Link href="/operator/create">
                ...
                <TableBody>
                  {operators.map((operator) => (
                    <TableRow key={operator.id}>
                      <TableCell>
                        <Link href={`/operator/${operator.address}`} className="font-medium hover:underline cursor-pointer">
